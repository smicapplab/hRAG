...
async function seedDemo() {
    ...
    // 1. Departments
    const [fieldOps] = await db.insert(schema.departments).values({ name: 'Field Operations' }).returning();
    const [hqAdmin] = await db.insert(schema.departments).values({ name: 'HQ Administration' }).returning();

    // 2. Groups
    const [logistics] = await db.insert(schema.groups).values({ name: 'Logistics Alpha', departmentId: fieldOps.id }).returning();
    const [legal] = await db.insert(schema.groups).values({ name: 'Legal & Compliance', departmentId: hqAdmin.id }).returning();
    const [intel] = await db.insert(schema.groups).values({ name: 'Intelligence Unit', departmentId: hqAdmin.id }).returning();
