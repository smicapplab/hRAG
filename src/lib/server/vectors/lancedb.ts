...
export class LanceDBVectorStore implements VectorStore {
	...
	async updateAccess(docId: string, accessIds: string[]) {
		...
		if (table) {
			...
			// Using JSON.stringify as LanceDB expects a string for the array column in some SDK versions.
			await table.update(
				{ access_ids: JSON.stringify(accessIds) },
				{ where: `doc_id = '${this.escape(docId)}'` }
			);
		}
	}
	...
	async search(
		...
		const results = await table.search(vector).where(whereClause).limit(limit).toArray();
